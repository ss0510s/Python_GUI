...
# 도움말 클릭 이벤트
def click_about():
    messagebox.showinfo('12191754 남수진', '12191754 정보통신공학과 남수진')
...
def run_progress(pb):
    ...
    hollys_df.to_csv('hollys.csv', mode='w', encoding='cp949', index=True) # 맥북 기준 encoding
...
# 학과 공지사항 정보 조회
def show_info(url):
    page = urllib.request.urlopen(url) # 학과 홈페이지
    soup = BeautifulSoup(page, "html.parser")
...
def change_number(ev):
    ...
    content = ''
    global class_name
    if class_name == '정보통신공학과':
        content = show_info(url='https://ice.inha.ac.kr/ice/2250/subview.do')
    elif class_name =='공간정보공학과':
        content = show_info(url='https://geoinfo.inha.ac.kr/geoinfo/2678/subview.do')
    lbl_content.configure(text=content)

# 정보통신공학과 공지사항 출력
def inf_content():
    global class_name
    class_name = '정보통신공학과'
    messagebox.showinfo('공지사항', class_name)
    url = 'https://ice.inha.ac.kr/ice/2250/subview.do'

    content = show_info(url)
    lbl_content.configure(text=content)
...
if __name__ == "__main__":
    ...
    inf_btn = ttk.Button(fr_tab1, text="정보통신공학과", command=inf_content)
    geo_btn = ttk.Button(fr_tab1, text="공간정보공학과", command=geo_content)

    lbl_disp = ttk.Label(fr_tab1, text="조회수 : ? (이상)")
    scaled = tk.DoubleVar()
    s = ttk.Scale(fr_tab1, variable=scaled, from_=0, to=200, orient='h', command=change_number)

    lbl_content = ttk.Label(fr_tab1, text='')
    inf_btn.pack(fill='x')
    geo_btn.pack(fill='x')
    lbl_disp.pack()
    s.pack(fill='x')
